...
import sys
import os
import perangkatputih

# Folder untuk menyimpan dataset
path_slice = "dataset/sliced/plat putih"
path_source = "dataset/source"
path_source_plat_putih = "dataset/source/plat_putih"

# Template untuk proyeksi vertikal
pv_template = Utils.proyeksi_vertical(cv2.imread("dataset/templates/plate/template-plat-putih.jpg", cv2.IMREAD_ANYCOLOR))

for file_name in sorted(os.listdir(path_source_plat_putih)):
    image = cv2.imread(os.path.join(path_source_plat_putih, file_name))
    src = image.copy()
    blurred = image.copy()
    print(image.shape)
